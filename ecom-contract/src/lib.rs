...
pub mod instruction;
use crate::instruction::EcomInstructionData;
...
pub fn process_instruction(
    ...
    let (variant, data) = instruction_data.split_first().ok_or(ProgramError::InvalidInstructionData)?;
    let instruction = EcomInstructionData::unpack(variant)?;
    ...
    match instruction {
        EcomInstructionData::InitializeAddressPDA => {
          msg!("Initialise Address PDAs");
        ...
        EcomInstructionData::UpdateAdddress => {
            ...
            let instruction_data_address = AddressSchema::try_from_slice(&data)?;
